...
#define STARTING_CAPACITY_BYTES 10
  ...
  typedef struct
  {
    ...
    size_t byte_i; // Current byte index - for readability only - functionally
                   // dependent on data_len_bits
   ...
   * @param nbits Number of bits that the element should occupy. More
   * significant 0's are filled in the left if [nbits] is greater than the
   * needed bits for elem
   * @return 0 if successful non zero else
   */
  int bits_data_push (bits_data *data, const uint64_t elem,
                      const size_t nbits);
   ...
   */
  uint64_t bits_data_get (const bits_data *data, const size_t start,
                          const size_t nbits);
   ...
   * @param len The length of dest (only for redundancy, only allowable lengths
   * are data.data_len_bits
   * @return 0 if successful, 1 else
   */
  int bits_data_unpack (const bits_data *data, uint8_t *dest,
                        const size_t len);
