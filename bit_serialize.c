...
extern "C"
{
   ...
   */
  static uint8_t
  get_u8_bit_left (const uint64_t _elem, const size_t bit)
  {
    if (bit < 8)
      {
  ...
  static int
  double_data_capacity (bits_data *data)
  {
  ...
  static int
  push_u8 (bits_data *data, const uint8_t num, const size_t nbits)
  {
  ...
  int
  bits_data_push (bits_data *data, const uint64_t _elem, const size_t _nbits)
  {
  ...
  uint64_t
  bits_data_get (const bits_data *data, const size_t _start,
                 const size_t _nbits)
  {
    assert (_nbits <= 64);
    uint64_t ret = 0;
  ...
  int
  bits_data_unpack (const bits_data *data, uint8_t *dest, const size_t len)
  {
    // This could be less restrictive by allowing len > data->data_len_bits;
